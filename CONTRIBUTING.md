...
- **The overlay is two files, and the chart-specific half is not inside the chart.** `scripts/ci/kubeconform-values.yaml` holds keys whose name *and* shape are the same fleet-wide; `scripts/ci/kubeconform-values/<chart>.yaml` is layered on top when it exists. Three charts have one — `v-rising` (`persistence` nested under `steamServer`/`world`, `service` under `server`/`rcon`, its empty probes, `externalIPs`), `nut-exporter` (`webconfig_file`, `extraRunFlags`), `exportarr` (its whole `exportarr:` subtree, which no top-level key can reach). **Nothing inside a chart directory tells you its overlay file exists, so check `scripts/ci/kubeconform-values/` when you touch a chart's passthroughs.** Both files sit outside `charts/` deliberately: `ct` globs `ci/*-values.yaml` and installs every match, so putting them there would require the Prometheus and Gateway CRDs in the kind cluster.
- **A green coverage guard does not mean full coverage.** `kubeconform.sh` fails when a chart declares a passthrough — a key defaulting to `{}`, `[]`, or bare with no children — that neither overlay file *populates*; an empty `key: {}` in the overlay does not count as covered. But it walks **one level**: top-level keys, plus one level under `exportarr:`. Nested passthroughs (`metrics.serviceMonitor.relabelings`, `service.rcon.externalIPs`, `exportarr.apps.<app>[].volumes`) are the larger half of the surface and are not checked — populate those by hand and cover them in unit tests. Keys defaulting to `""` are invisible to it too, which is why credential-gated templates like `secret.yaml` rely on helm-unittest.
- **Two anti-empty-render checks, because `kubeconform` reports `Valid: 0, Errors: 0` on no input at all.** Pass 2 must render at least one resource — pass 1 may legitimately render none (`exportarr` has every app disabled by default). Separately, the script fails if a chart ships a CRD template the overlay didn't reach — but that guard matches on *kind* for three hard-coded filenames, so it does not extend to core kinds.
