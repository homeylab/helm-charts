...
![Version: 1.2.0](https://img.shields.io/badge/Version-1.2.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 3.0.0](https://img.shields.io/badge/AppVersion-3.0.0-informational?style=flat-square)
...
### Health Endpoint And Probes
Probes apply to **Deployment mode only**, and only when `config.health_port` is set — upstream starts no health server otherwise and ignores the setting in one-shot (CronJob) mode. When it is set, the chart names a container port `health` and renders `livenessProbe` / `readinessProbe` against `GET /healthz` on it.

Both are plain values passthroughs (`livenessProbe`, `readinessProbe`), defaulting to `initialDelaySeconds: 10` / `periodSeconds: 30`. Set one to `null` to drop it — `{}` does **not** work, since Helm merges an empty map into the default rather than replacing it.

`/healthz` is a *liveness* signal: it stays `200` after a failed export cycle, so probes don't flap on a transient BookStack outage. Alert on the response body's `last_run.status` (`degraded` for a partial run) or `failure_count` instead.
