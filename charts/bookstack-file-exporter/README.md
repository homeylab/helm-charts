...
Probes render in **Deployment mode only**, and only with `config.health_port` set — upstream starts no health server otherwise, and ignores it in CronJob mode. The chart then names a container port `health` and points the `livenessProbe` / `readinessProbe` passthroughs (default `initialDelaySeconds: 10` / `periodSeconds: 30`) at `GET /healthz` on it; set one to `null` (not `{}`, which Helm merges into the default) to drop it.

`/healthz` is a *liveness* signal — it stays `200` after a failed export cycle, so probes don't flap on a transient BookStack outage. Alert instead on the body's `last_run.status` (`degraded` for a partial run) or `failure_count`.
