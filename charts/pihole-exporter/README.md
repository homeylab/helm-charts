...
> [!IMPORTANT]
> **Chart `1.0.0` moves credentials into a chart-managed Secret.** This is a breaking change:
> - `settings.auth.password` / `settings.auth.token` are no longer passed as plaintext Deployment env; the chart now renders a `Secret` (`templates/secret.yaml`) and wires it via `envFrom`. Behavior is preserved — the value still lands as `PIHOLE_PASSWORD`.
> - `settings.auth.existingSecret` still overrides the managed Secret; the secret must provide the `PIHOLE_PASSWORD` key.
...
| ------------------- | -------------------- | ------------- |
| `0.1.X` | `1.0.0` | Credentials moved from plaintext Deployment env into a chart-managed Secret (`templates/secret.yaml`), wired via `envFrom`. If you set `settings.auth.password`/`token`, no action is needed — the value now lands in a Secret. `settings.auth.existingSecret` users are unaffected. New optional `httproute.*` (Gateway API v1) and hardened `securityContext` defaults were added. |
